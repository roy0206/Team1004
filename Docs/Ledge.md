...
| `spawnSafeLead` | **`5.5`** | 다음 단차 도착까지 이 시간 이하로 남으면 조각이 아직 활성되지 않았어도 **미리** 장애물 스폰을 끊는다(`LedgeDirector.IsNextLedgeNear`). 2026-09-06 긴 바위 패스에서 3.5 → 5.5로 올렸다. 아래 「단차 앞 스폰 정지」 |
| `postClearSafeTime` | `2.0` | 단차를 넘은 뒤 이만큼 더 스폰을 멈춰 둔다 |
...
| `clearMargin` | **`3.35`** | 긴 돌의 폭. 착지 시점에 앞면이 `playerX − 3.35`보다 오른쪽이면 아직 못 넘은 것으로 보고 **실패**로 본다. 2026-09-06 전신 높이 패스에서 0.98 → 3.35로 커진 폭에 맞춰 1.2에서 올렸다(「단차 모양」) |
...
## 단차 앞 스폰 정지 (`spawnSafeLead`, 2026-09-06 긴 바위 패스에서 3.5 → 5.5)

`LedgeDirector.ApplyWorldState`가 세 가지 중 하나라도 참이면 `IsSpawnSuspended`를 켜고 `PlayFlow`가 `ObstacleSpawner.SetSpawningEnabled(false)`를 부른다: 활성 조각이 접근 중(`approachSafeTime`), 넘은 직후 유예 중(`postClearSafeTime`), **다음 단차가 `spawnSafeLead` 안에 들어옴**(`IsNextLedgeNear`).

정지는 **새 스폰만** 막는다. 이미 떠 있는 장애물은 그대로 흘러오므로, 리드는 「정지 직전에 결정된 마지막 패턴이 단차 QTE 전에 완전히 끝나는」 길이여야 한다. 일반 구간에 긴 바위(점프 필수)가 들어오면서 이 계산이 빡빡해졌다.

```text
마지막 결정 시각        = ledgeTime − spawnSafeLead
긴 바위 도착           = 결정 + 2.59초        (EarliestArrivalDelay, Docs/Spawner.md)
가장 늦은 점프 시작     = 도착 시점
착지                  = 점프 시작 + 1.6초
QTE 창이 열리는 시각    = ledgeTime − 0.625    (qteStartDistance 2.5 ÷ 스크롤 4)

리드 3.5 → 착지 ledgeTime + 0.02   ← QTE 창과 겹친다. 공중이면 QTE가 열리지 않고,
                                     열려도 착지가 늦어 clearMargin 판정에 걸린다
리드 5.5 → 착지 ledgeTime − 1.31   ← 착지가 QTE 창보다 0.69초 이르다
```

리드를 올려 잃는 것은 크지 않다. 구간 1은 어차피 5초·12.5초 두 단차가 앞머리를 먹어 장애물이 16.5초쯤부터 나오고(3.5일 때 15.2초), 구간 2는 9.5~11.5초 구간의 패턴 한 개를 잃는다. 긴 바위를 일반 구간에서 빼면(`jumpRequiredRatio` 0) 3.5로 되돌려도 된다.
...
**2026-09-06 전신 높이 패스**: 「긴 돌」이 맨 아래 레인 한 칸(높이 1.1)만 채우던 것을 **강바닥에서 수면 위까지 솟은 전신**(높이 3.77)으로 키웠다. v8 기획서 `05_긴바위_단차_일반기믹.md`의 공통 리소스 규정(「강바닥에서 수면까지 솟은 형태」)을 따른 것이고, 일반 구간 장애물 `LongRock`(Variant B)과 **같은 스케일·같은 세로 위치**다(`Docs/Spawner.md` 「긴 바위」).

| 오브젝트 | 스프라이트 | 크기(unit) | 중심(로컬) | 정렬 | 뜻 |
| --- | --- | --- | --- | --- | --- |
| `UpperBed` | `Placeholder/Env_Riverbed.png` | Tiled 30 × 5 | (15, −2.75, z −0.005) | 7 | 위쪽 강바닥(모래) |
| `Step` (옛 `Cascade` 자리) | **`Art/오브젝트/긴돌.png`** | Simple **3.34896 × 3.77** (균등 스케일 **0.4839538**) | (**1.67448**, **0.165**, z **−0.01**) | 7 | 단차 앞면의 긴 돌. 윗변 **2.05**, 아랫변 **−1.72** |
| `Hint` | `Ledge/Ledge_Hint.png` | 1.0 × 1.2 | (−2.2, 0.6) | −4 | `↑`. 맨 처음 조각에만 연결 |

- **크기 계산.** `긴돌.png`의 알파 박스는 **692 × 779 px**(1920×1080 캔버스)이고 pivot은 알파 박스 중심 `(0.6557292, 0.36064816)`이다(`ObjectArtPostprocessor`가 임포트마다 다시 건다 — `Docs/Animation.md` 「후처리기 범위」). 아랫변을 강바닥(**−1.65**)에 0.07 묻고(**−1.72**) 윗변을 수면(**2.0**) 위로 0.05 내면(**2.05**) 높이가 **3.77**이다.

```text
균등 스케일 = 3.77 / (779/100)       = 0.4839538
폭          = (692/100) × 0.4839538  = 3.34896
중심 y      = (2.05 + (−1.72)) / 2   = 0.165
중심 x      = 3.34896 / 2            = 1.67448   (왼쪽 변이 단차 앞면 x 0에 온다)
```

- **`clearMargin`을 3.35로 올렸다.** `LedgeClock`은 착지 시점에 앞면(돌의 왼쪽 변) x가 `playerX − clearMargin`보다 오른쪽이면 아직 못 넘은 것으로 보고 실패시킨다. 즉 `clearMargin`은 **돌의 폭**이어야 「돌이 완전히 지나갔다」와 판정이 맞는다. 폭이 0.9772 → 3.34896으로 커졌으므로 1.2 → **3.35**다.
  - QTE 성공 점프는 도착 **0.625초 전 ~ 도착 시점** 사이에 나가고 체공이 1.6초이므로, 가장 이른 착지도 도착 **0.975초 뒤** = 앞면 `playerX − 3.9`다. 3.9 > 3.35이라 정상 경로는 언제나 넘는다(여유 0.55 unit = 0.14초). 테스트 `QteJump_LandsAfterTheWiderWallHasPassed`.
  - 반대로 **손으로 너무 일찍 뛰면 예전보다 쉽게 실패한다.** 안전한 점프 시작은 도착 **0.7625초 전 이후**다(1.6 − 3.35/4). 예전(폭 0.98)에는 1.35초 전까지 괜찮았다. QTE 창이 0.625초 전에 열리므로 그 창을 기다린 플레이어에게는 영향이 없다.
- **정렬과 z.** `UpperBed`와 같은 정렬 7이고 z만 −0.01로 더 앞이라 위쪽 강바닥보다 앞에 그려진다. 연어는 정렬 10이라 **돌 앞**을 지난다. 모래(7)가 돌 아랫부분을 덮어 −1.72 ~ −1.65 구간이 묻힌다.
- **`WaterInteractor`가 하나도 없다.** 단차 조각은 판정도 물 반응도 로직으로만 다룬다. 돌 윗변이 이제 수면 위(2.05)로 나오지만, 일부러 물 항적을 붙이지 않았다 — 단차는 월드가 통째로 0.7초 동안 1.1 내려가는 연출을 하므로 항적을 넣으면 그때 수면이 요동친다(예전 `Cascade`·`Foam`의 `WaterInteractor`를 지운 이유와 같다, `Docs/Environment.md`). 일반 구간 장애물 `LongRock`은 아래로 내려가지 않으므로 항적을 붙였다.
- **`UpperBed` 중심 y가 −2.85 → −2.75로 바뀌어 있다**(사용자가 손으로 조정 중인 값이다). 그러면 모래 윗선이 −0.45가 된다. 돌이 이제 −1.72에서 2.05까지라 그 선을 세로로 통과하므로, 예전처럼 돌 윗변과 모래 윗선을 맞출 필요가 없어졌다.
- **넘은 뒤 0.7초 하강에서 돌이 보인다.** `Cleared` 시점의 앞면은 `playerX − 3.35` = −7.55라 돌의 오른쪽 변이 −4.2다(화면 왼쪽 끝 −6.4). 상승(= 조각 하강) 첫 0.55초 동안 화면 왼쪽에서 돌이 가라앉는 것이 보인다. 예전에도 같은 상황이었지만 돌이 3배 높아져 더 눈에 띈다 — 어색하면 `clearMargin`을 폭보다 조금 크게 두어 더 왼쪽에서 `Cleared`가 나게 한다.
...
LedgeSet (LedgeDirector: data, ledges[3], environment=null, player=null)
  Ledge1 (LedgeThing, 비활성, parkX 60)          구간 1 · 5초 (힌트 있음)
    ...
    Step      Simple 3.34896 × 3.77, 중심 (1.67448, 0.165, z -0.01), 정렬 7, 균등 스케일 0.4839538
              긴돌.png. 윗변 2.05 / 아랫변 -1.72. WaterInteractor 없음
...
### 2026-09-06 전신 높이 패스 (가장 최근)

v8 `05_긴바위_단차_일반기믹.md`의 공통 리소스 규정과 사용자 결정(Variant B 채택)에 맞춰 단차 앞면을 강바닥~수면 전신으로 키웠다. 에디터가 열려 있어 생성기·Unity 테스트는 돌리지 않았다.

| 한 일 | 내용 |
| --- | --- |
| `LedgeSet.prefab` | 조각 3개의 `Step` transform: `(0.4886, −1.1, −0.01)` / 스케일 0.1412067 → **`(1.67448, 0.165, −0.01)` / 스케일 0.4839538**. 다른 것은 건드리지 않았다 |
| `LedgeData.asset` | `clearMargin` 1.2 → **3.35**(돌 폭), `spawnSafeLead` 3.5 → **5.5**(일반 구간 긴 바위와 QTE가 겹치지 않게) |
| `LedgeData.cs` | 같은 두 값의 직렬화 기본값도 맞췄다 |
| 테스트 | `LedgeScheduleTests`의 `ClearMargin` 상수 1.2 → 3.35, 「공중으로 넘는」 4개 시나리오의 체공을 1.0 → 1.5초로 늘렸다(넓어진 폭을 넘으려면 필요하다). 새 테스트 2개: `Step_FillsTheRiverbedToTheWaterSurface`(스케일·폭·중심·강바닥/수면 관계), `QteJump_LandsAfterTheWiderWallHasPassed`(QTE 성공 점프가 가장 이른 경우에도 넘는다) |
| 확인 | 오프라인 Roslyn 컴파일 `assemblies compiled: 44  failed: 0`, 프리팹 앵커·GUID·자식 링크 검사 통과 |

에디터에서 확인할 것: 단차 앞면의 돌이 강바닥에서 수면 위까지 솟는지, 연어(정렬 10)가 돌 앞을 지나는지, 모래가 돌 아랫부분만 덮는지, QTE 성공 점프로 넘은 뒤 `Failed`가 나지 않는지, 넘은 뒤 0.7초 하강에서 화면 왼쪽의 돌이 가라앉는 그림이 어색하지 않은지, 구간 1·2에서 단차 앞뒤로 장애물이 더 일찍 끊기는지(리드 5.5초).
...
10. **긴 돌(전신)**: 돌이 강바닥(−1.65)에서 수면(2.0) 위까지 한 덩이로 솟는가. 아랫변이 모래에 조금 묻히고 윗변이 물 밖으로 나오는가. 연어가 돌 **앞**을 지나는가(정렬 10 vs 7). 폭 3.35가 화면에서 너무 두껍지 않은가. 넘은 뒤 하강 0.7초에 화면 왼쪽에서 돌이 가라앉는 그림이 어색하지 않은가.
...
- **너무 일찍 뛴 경우가 이제 실패다.** 도착 직전에 뛰어 도착 직후 착지하면 앞면이 아직 `playerX − clearMargin` 안이라 `Failed`가 된다(예전에는 다시 막혔다). QTE 성공 점프는 도착까지 최대 0.625초 남은 시점에 나가고 체공이 1.6초이므로 **정상 경로에서는 나올 수 없다**. 손으로 미리 뛴 경우에만 걸린다. **`clearMargin`이 1.2 → 3.35로 커져 이 창이 좁아졌다**: 안전한 손 점프는 도착 0.7625초 전 이후다(예전 1.35초). 「QTE 창 전에 미리 뛰어 넘기」를 넓게 열어 두려면 `clearMargin`을 돌 폭보다 작게 두면 되지만, 그러면 돌이 아직 연어에 걸쳐 있는데 통과 판정이 난다.
...
- **긴 돌이 전신 높이가 되면서 폭이 3.35가 되었다.** 예전 「윗변 −0.55, 폭 0.98」이 「윗변 2.05, 폭 3.35」다. 넘어야 할 턱의 논리적 높이(`laneY[0] + stepHeight` = 2.2)와 그림 윗변(2.05)이 이제 거의 같아 「턱을 실제로 넘는」 그림이 되었지만, 앞면이 3.35 unit(화면 폭 12.8의 26%)으로 두꺼워졌다. 얇게 보이길 원하면 세로로 더 긴 아트가 필요하다(`Docs/ArtRequestList.md` 4번).
